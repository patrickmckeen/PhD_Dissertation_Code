...
def run_sim(orb,state0,real_sat,adcsys,j2000_0 = 0.22,tf=60*60*3,dt = 1,alt_title = None,rand = False):
    ...
    orb_hist = []
    est_orb_hist = []
    control_hist = np.nan*np.zeros((int((tf-t0)/dt),adcsys.virtual_sat.control_len))
    cov_hist = []
    t_hist = np.nan*np.zeros(int((tf-t0)/dt))
    ...
    est_sat0 = copy.deepcopy(adcsys.virtual_sat)
    ...
    try:
        while t<tf:
            ...
            adcsys.orbital_state_update(j2000_0+(t-t0)*sec2cent)
            adcsys.estimation_update(control,sens)
            est_state = adcsys.estimator.use_state.val
            prev_control = control.copy()
            control = adcsys.actuation()
            ...
            state_err = adcsys.current_law.state_err(state,goal_state,print_info=False)
            print('ctrlquaterr ',state_err[3:7])
            est_state_err = adcsys.current_law.state_err(est_state,goal_state,print_info=False)
            ...
            est_state_hist[ind,:] = adcsys.estimator.use_state.val
            goal_hist += [goal_state.copy()]
            orb_hist += [orbt]
            est_orb_hist += [adcsys.orbital_state]
            control_hist[ind,:] = prev_control
            cov_hist += [adcsys.estimator.use_state.cov]
            t_hist[ind] = t
            # comp_time_hist[ind] = (tt1-tt0)
            dist_torq_hist[ind,:] = real_sat.last_dist_torq.copy()
            est_dist_torq_hist[ind,:] = adcsys.virtual_sat.dist_torque(prev_est_state[:adcsys.virtual_sat.state_len],adcsys.estimator.prev_os_vecs).copy()
            act_torq_hist[ind,:] = real_sat.last_act_torq.copy()
            est_act_torq_hist[ind,:] =  adcsys.virtual_sat.act_torque(prev_est_state[:adcsys.virtual_sat.state_len],prev_control,adcsys.estimator.prev_os_vecs,False)
            ...
            adcsys.virtual_sat.dynamics(adcsys.estimator.use_state.val[:7],adcsys.current_ctrl,adcsys.orbital_state,verbose = False,save_dist_torq = True,save_act_torq = True,update_actuator_noise = False,save_details = True)
    ...
    # sens_biases = np.array([j.has_bias*j.output_len for j in range(len(adcsys.virtual_sat.attitude_sensors)) if isinstance(adcsys.virtual_sat.attitude_sensors[j],SunSensorPair) and adcsys.virtual_sat.attitude_sensors[j].has_bias and adcsys.virtual_sat.attitude_sensors[j].estimated_bias])

    est_sb_inds = (np.array([np.sum([k.output_length for k in adcsys.virtual_sat.attitude_sensors[:j] if k.estimated_bias and k.has_bias]) for j in range(len(adcsys.virtual_sat.attitude_sensors)) if isinstance(adcsys.virtual_sat.attitude_sensors[j],SunSensorPair) and adcsys.virtual_sat.attitude_sensors[j].has_bias and adcsys.virtual_sat.attitude_sensors[j].estimated_bias]) + adcsys.virtual_sat.state_len+adcsys.virtual_sat.act_bias_len).astype('int')
    est_mb_inds = (np.array([np.sum([k.output_length for k in adcsys.virtual_sat.attitude_sensors[:j] if k.estimated_bias and k.has_bias]) for j in range(len(adcsys.virtual_sat.attitude_sensors)) if isinstance(adcsys.virtual_sat.attitude_sensors[j],MTM) and adcsys.virtual_sat.attitude_sensors[j].has_bias and adcsys.virtual_sat.attitude_sensors[j].estimated_bias]) + adcsys.virtual_sat.state_len+adcsys.virtual_sat.act_bias_len).astype('int')
    est_gb_inds = (np.array([np.sum([k.output_length for k in adcsys.virtual_sat.attitude_sensors[:j] if k.estimated_bias and k.has_bias]) for j in range(len(adcsys.virtual_sat.attitude_sensors)) if isinstance(adcsys.virtual_sat.attitude_sensors[j],Gyro) and adcsys.virtual_sat.attitude_sensors[j].has_bias and adcsys.virtual_sat.attitude_sensors[j].estimated_bias]) + adcsys.virtual_sat.state_len+adcsys.virtual_sat.act_bias_len).astype('int')
    est_ab_inds = (np.array([np.sum([k.input_len for k in adcsys.virtual_sat.actuators[:j] if k.estimated_bias and k.has_bias]) for j in range(len(adcsys.virtual_sat.actuators)) if adcsys.virtual_sat.actuators[j].has_bias and adcsys.virtual_sat.actuators[j].estimated_bias]) + adcsys.virtual_sat.state_len).astype('int')
    try:
        est_dipole_inds = (np.concatenate([np.sum([k.main_param.size for k in adcsys.virtual_sat.disturbances[:j] if k.estimated_param]) for j in range(len(adcsys.virtual_sat.disturbances)) if adcsys.virtual_sat.disturbances[j].estimated_param and isinstance(adcsys.virtual_sat.disturbances[j],Dipole_Disturbance)]) + adcsys.virtual_sat.state_len+adcsys.virtual_sat.act_bias_len+adcsys.estimator.att_sens_bias_len).astype('int')
    except:
        est_dipole_inds = np.array([])
    try:
        est_gendist_inds = (np.concatenate([np.sum([k.main_param.size for k in adcsys.virtual_sat.disturbances[:j] if k.estimated_param]) for j in range(len(adcsys.virtual_sat.disturbances)) if adcsys.virtual_sat.disturbances[j].estimated_param and isinstance(adcsys.virtual_sat.disturbances[j],General_Disturbance)]) + adcsys.virtual_sat.state_len+adcsys.virtual_sat.act_bias_len+adcsys.estimator.att_sens_bias_len).astype('int')
    except:
    ...
    info = [base_title,adcsys.estimator.al,adcsys.estimator.kap,adcsys.estimator.bet,dt,werrcov,mrperrcov,1,real_sat.J]+[np.diag(cov_estimate.copy()),int_cov.copy()]+[ctrlmetrics.time_to_conv,ctrlmetrics.tc_est,np.mean(ctrlangdiff[-100:]),np.amax(ctrlangdiff[-100:])]
    ...
    sim.est_sat_J = adcsys.virtual_sat.J
    # try:
...
if __name__ == '__main__':
    ...
    wie_disturbed_sat = create_Wie_sat(real=True,rand=False,use_gg = True, use_drag = True, use_dipole = True, use_SRP = True,include_magicbias = True,include_magic_noise = True,include_mtmbias = True)
    wie_disturbed_est_sat = create_Wie_sat(real=False,rand=False,use_gg = True, use_drag = True, use_dipole = True, use_SRP = True,estimate_dipole = True,include_magicbias = True,estimate_magic_bias = True,include_magic_noise = True,include_mtmbias = True)
    wie_disturbed_gen_sat = create_Wie_sat(real=True,rand=False,use_gg = True, use_drag = True, use_dipole = True, use_SRP = True,include_magicbias = True,include_magic_noise = True,include_mtmbias = True,estimate_mtm_bias = True)
    wie_disturbed_genest_sat = create_Wie_sat(real=False,rand=False,use_gg = False, use_drag = False, use_dipole = False, use_SRP = False,use_gen = True,estimate_dipole = False,estimate_gen_torq = True,include_magicbias = True,estimate_magic_bias = True,gen_torq_std = 1e-5, gen_mag_max = 1e-2,include_magic_noise = True,include_mtmbias = True,estimate_mtm_bias = True)
    ...
    wie_disturbed_cov0_estimate = block_diag(np.eye(3)*avcov,np.eye(3)*angcov,np.eye(3)*2*2,np.eye(3)*1e-6**2,np.eye(3)*(2.5*((math.pi/180.0)/3600))**2.0,np.eye(3)*(0.3*0.02)**2.0,np.eye(3)*(3)**2.0)
    wie_gendisturbed_cov0_estimate = block_diag(np.eye(3)*avcov,np.eye(3)*angcov,np.eye(3)*2*2,np.eye(3)*1e-6**2,np.eye(3)*(2.5*((math.pi/180.0)/3600))**2.0,np.eye(3)*(0.3*0.02)**2.0,np.eye(3)*(1e-2**2.0))
    ...
    wie_match_10 = ["Wie_matching",          wie_base_est_sat,           wie_base_sat,       10, wie_w0,         wie_q0,         wie_base_cov0_estimate,         False,  wie_goals,         2*60*60,    wie_orb_file,[1.0,1.0,1.0]]
    wie_match_1 = ["Wie_matching1",          wie_base_est_sat,           wie_base_sat,       1,  wie_w0,         wie_q0,         wie_base_cov0_estimate,         False,  wie_goals,         2*60*60,    wie_orb_file,[1.0,1.0,1.0]]
    lovera_match_10 = ["Lovera_matching",           lovera_base_est_sat,        lovera_base_sat,    10, lovera_w0,      lovera_q0,      lovera_base_cov0_estimate,      False,  lovera_goals,      12*60*60,    lovera_orb_file,[1.0,1.0,1.0]]
    lovera_match_1 = ["Lovera_matching1",           lovera_base_est_sat,        lovera_base_sat,    1,  lovera_w0,      lovera_q0,      lovera_base_cov0_estimate,      False,  lovera_goals,      10*60*60,    lovera_orb_file1,[1.0,1.0,1.0]]
    ...
    wie_disturbed_10 = ["Wie_disturbed", wie_disturbed_est_sat,wie_disturbed_sat,10,wie_w0,wie_q0,wie_disturbed_cov0_estimate,False,wie_goals,2*60*60,wie_orb_file,[1.0,1.0,1.0]]
    wie_disturbed_1 = ["Wie_disturbed1", wie_disturbed_est_sat,wie_disturbed_sat,1,wie_w0,wie_q0,wie_disturbed_cov0_estimate,False,wie_goals,2*60*60,wie_orb_file,[1.0,1.0,1.0]]
    ...
    wie_disturbed_w_control10 = ["Wie_disturbed_w_control", wie_disturbed_est_sat,wie_disturbed_sat,10,wie_w0,wie_q0,wie_disturbed_cov0_estimate,True,wie_goals,2*60*60,wie_orb_file,[1.0,1.0,1.0]]
    wie_disturbed_w_control1 = ["Wie_disturbed_w_control1", wie_disturbed_est_sat,wie_disturbed_sat,1,wie_w0,wie_q0,wie_disturbed_cov0_estimate,True,wie_goals,2*60*60,wie_orb_file,[1.0,1.0,1.0]]
    ...
    wie_disturbed_w_gencontrol10 = ["Wie_disturbed_w_gencontrol", wie_disturbed_genest_sat,wie_disturbed_gen_sat,10,wie_w0,wie_q0,wie_gendisturbed_cov0_estimate,True,wie_goals,2*60*60,wie_orb_file,[1.0,1.0,1.0]]
    wie_disturbed_w_gencontrol1 = ["Wie_disturbed_w_gencontrol1", wie_disturbed_genest_sat,wie_disturbed_gen_sat,1,wie_w0,wie_q0,wie_gendisturbed_cov0_estimate,True,wie_goals,2*60*60,wie_orb_file,[1.0,1.0,1.0]]
    ...
    tests = [wie_disturbed_1,wie_disturbed_w_control1,wie_disturbed_w_gencontrol1]
    ...
    for j in tests:
        ...
        est.kap =  3 - (estimate.size - 1 - sum([j.use_noise for j in adcsys.virtual_sat.actuators]))
        ...
        adcsys = ADCS(est_sat,orbit_estimator,est,control_laws,use_planner = False,planner = None,planner_settings = None,goals=None,prop_schedule=None,dt = 1,control_dt = None,tracking_LQR_formulation = 0)
