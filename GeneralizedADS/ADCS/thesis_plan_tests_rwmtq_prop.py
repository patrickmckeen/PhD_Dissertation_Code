...
main_sat_rwmtq =  create_GPS_6U_sat(real=True,rand=False,mtm_scale = mtm_scale,use_dipole = dipole_on,use_mtq = True,include_mtmbias = mtm_bias_on, include_mtqbias = mtq_bias_on,include_gbias = gyro_bias_on,include_sbias = sun_bias_on,use_prop = prop_torq_on,prop_torq0 = 1e-4*normalize(np.array([-3,-8,1])))
...
main_sat_mtq =  create_GPS_6U_sat(real=True,rand=False,mtm_scale = mtm_scale,use_dipole = dipole_on,use_mtq = True,include_mtmbias = mtm_bias_on, include_mtqbias = mtq_bias_on,include_gbias = gyro_bias_on,include_sbias = sun_bias_on,use_prop = prop_torq_on,use_RW = False,prop_torq0 = 1e-5*normalize(np.array([0,-8,1])))
...
main_sat_mtq1rw =  create_GPS_6U_sat_1RW(real=True,rand=False,mtm_scale = mtm_scale,use_dipole = dipole_on,use_mtq = True,include_mtmbias = mtm_bias_on, include_mtqbias = mtq_bias_on,include_gbias = gyro_bias_on,include_sbias = sun_bias_on,use_prop = prop_torq_on,use_RW = True,prop_torq0 = 1e-5*normalize(np.array([0,-8,1])))
...
plan_rwmtq_goals = Goals({0.2:GovernorMode.NO_CONTROL,0.22+5*sec2cent:GovernorMode.RWBDOT_WITH_EKF,0.22+100*sec2cent:GovernorMode.PLAN_AND_TRACK_MPC},
                {0.2:(PointingGoalVectorMode.ANTI_RAM,np.zeros(3)),0.22+1200*sec2cent:(PointingGoalVectorMode.NADIR,np.zeros(3)),0.22+2500*sec2cent:(PointingGoalVectorMode.ANTI_RAM,np.zeros(3))},
                {0.22:-unitvecs[0],0.22+1200*sec2cent:unitvecs[2],0.22+2500*sec2cent:-unitvecs[0]})

plan_rwmtq_goals_LQR = Goals({0.2:GovernorMode.NO_CONTROL,0.22+5*sec2cent:GovernorMode.RWBDOT_WITH_EKF,0.22+100*sec2cent:GovernorMode.PLAN_AND_TRACK_LQR},
                {0.2:(PointingGoalVectorMode.ANTI_RAM,np.zeros(3)),0.22+1200*sec2cent:(PointingGoalVectorMode.NADIR,np.zeros(3)),0.22+2500*sec2cent:(PointingGoalVectorMode.ANTI_RAM,np.zeros(3))},
                {0.22:-unitvecs[0],0.22+900*sec2cent:unitvecs[2],0.22+2500*sec2cent:-unitvecs[0]})
...
plan_mtq_goals = Goals({0.2:GovernorMode.NO_CONTROL,0.22+5*sec2cent:GovernorMode.BDOT_WITH_EKF,0.22+100*sec2cent:GovernorMode.PLAN_AND_TRACK_MPC},
                {0.2:(PointingGoalVectorMode.ANTI_RAM,np.zeros(3)),0.22+1100*sec2cent:(PointingGoalVectorMode.NO_GOAL,np.zeros(3)),0.22+1200*sec2cent:(PointingGoalVectorMode.NADIR,np.zeros(3)),0.22+2400*sec2cent:(PointingGoalVectorMode.NO_GOAL,np.zeros(3)),0.22+2500*sec2cent:(PointingGoalVectorMode.ANTI_RAM,np.zeros(3))},
                {0.22:-unitvecs[0],0.22+1200*sec2cent:unitvecs[2],0.22+2500*sec2cent:-unitvecs[0]})
...
plan_mtq1rw_goals = Goals({0.2:GovernorMode.NO_CONTROL,0.22+5*sec2cent:GovernorMode.BDOT_WITH_EKF,0.22+100*sec2cent:GovernorMode.PLAN_AND_TRACK_MPC},
                {0.2:(PointingGoalVectorMode.ANTI_RAM,np.zeros(3)),0.22+1200*sec2cent:(PointingGoalVectorMode.NADIR,np.zeros(3)),0.22+2500*sec2cent:(PointingGoalVectorMode.ANTI_RAM,np.zeros(3))},
                {0.22:-unitvecs[0],0.22+1200*sec2cent:unitvecs[2],0.22+2500*sec2cent:-unitvecs[0]})
...
prop_schedule = {0.22+20*sec2cent:True,0.22+30*sec2cent:False,0.22+400*sec2cent:True,0.22+1100*sec2cent:False,0.22+2800*sec2cent:True}
...
if mtm_bias_on:
    cov0_estimate_only_gen_rwmtq = block_diag(base_cov,np.eye(3*mtm_bias_on)*(5e-7)**2.0*np.diagflat([j.scale for j in main_sat_rwmtq.sensors if isinstance(j,MTM)]),np.eye(3*gyro_bias_on)*(0.2*((math.pi/180.0)))**2.0,np.eye(3*sun_bias_on)*(0.3*0.2)**2.0,np.eye(3)*(5e-3)**2.0)
    cov0_estimate_only_gen_mtq = block_diag(base_cov_norw,np.eye(3*mtm_bias_on)*(5e-7)**2.0*np.diagflat([j.scale for j in main_sat_rwmtq.sensors if isinstance(j,MTM)]),np.eye(3*gyro_bias_on)*(0.2*((math.pi/180.0)))**2.0,np.eye(3*sun_bias_on)*(0.3*0.2)**2.0,np.eye(3)*(5e-3)**2.0)
    cov0_estimate_only_gen_mtq_1rw = block_diag(base_cov_1rw,np.eye(3*mtm_bias_on)*(5e-7)**2.0*np.diagflat([j.scale for j in main_sat_rwmtq.sensors if isinstance(j,MTM)]),np.eye(3*gyro_bias_on)*(0.2*((math.pi/180.0)))**2.0,np.eye(3*sun_bias_on)*(0.3*0.2)**2.0,np.eye(3)*(5e-3)**2.0)

else:
    cov0_estimate_only_gen_mtq = block_diag(base_cov_norw,np.eye(3*gyro_bias_on)*(0.2*((math.pi/180.0)))**2.0,np.eye(3*sun_bias_on)*(0.3*0.2)**2.0,np.eye(3)*(5e-3)**2.0)
    cov0_estimate_only_gen_mtq_1rw = block_diag(base_cov_1rw,np.eye(3*gyro_bias_on)*(0.2*((math.pi/180.0)))**2.0,np.eye(3*sun_bias_on)*(0.3*0.2)**2.0,np.eye(3)*(5e-3)**2.0)

    cov0_estimate_only_gen_rwmtq = block_diag(base_cov,np.eye(3*gyro_bias_on)*(0.2*((math.pi/180.0)))**2.0,np.eye(3*sun_bias_on)*(0.3*0.2)**2.0,np.eye(3)*(5e-3)**2.0)
...
case_quat_RWMTQ_strong_prop = ["thesis_6U_quat_RWMTQ_strongprop",           est_sat_w_all_rwmtq,     main_sat_rwmtq,    1,  w0,      q0,h0,      cov0_estimate_all_rwmtq,      True,  plan_rwmtq_goals, prop_schedule,      4500,    orb_file,all_int_cov_rwmtq,3*(mtm_bias_on+gyro_bias_on+sun_bias_on),3*(prop_torq_on+dipole_on),3*mtq_bias_on,1e-3,0.0,"",0.0,1.0,w0,q0,h0]
case_quat_RWMTQ_strong_prop_gen = ["thesis_6U_quat_RWMTQ_strongprop_wgen",           est_sat_only_gen_rwmtq,     main_sat_rwmtq,    1,  w0,      q0,h0,      cov0_estimate_only_gen_rwmtq,      True,  plan_rwmtq_goals, prop_schedule,      4500,    orb_file,all_int_cov_rwmtq,3*(mtm_bias_on+gyro_bias_on+sun_bias_on),3*(prop_torq_on),3*mtq_bias_on,1e-3,0.0,"",0.0,1.0,w0,q0,h0]
case_quat_RWMTQ_strong_prop_LQR = ["thesis_6U_quat_RWMTQ_strongprop_LQR",           est_sat_w_all_rwmtq,     main_sat_rwmtq,    1,  w0,      q0,h0,      cov0_estimate_all_rwmtq,      True,  plan_rwmtq_goals_LQR, prop_schedule,      4500,    orb_file,all_int_cov_rwmtq,3*(mtm_bias_on+gyro_bias_on+sun_bias_on),3*(prop_torq_on+dipole_on),3*mtq_bias_on,1e-3,0.0,"",0.0,1.0,w0,q0,h0]

case_quat_MTQ_strong_prop_LQR = ["thesis_6U_quat_MTQ_strongprop_LQR",           est_sat_w_all_mtq,     main_sat_mtq,    1,  w0,      q0,[],      cov0_estimate_all_mtq,      True,  plan_mtq_goals_LQR, prop_schedule,      4500,    orb_file,all_int_cov_mtq,3*(mtm_bias_on+gyro_bias_on+sun_bias_on),3*(prop_torq_on+dipole_on),3*mtq_bias_on,1e-3,0.0,"",0.0,1.0,w0,q0,[]]
case_quat_MTQ_strong_prop = ["thesis_6U_quat_MTQ_strongprop",           est_sat_w_all_mtq,     main_sat_mtq,    1,  w0,      q0,[],      cov0_estimate_all_mtq,      True,  plan_mtq_goals, prop_schedule,      4500,    orb_file,all_int_cov_mtq,3*(mtm_bias_on+gyro_bias_on+sun_bias_on),3*(prop_torq_on+dipole_on),3*mtq_bias_on,1e-3,0.0,"",0.0,1.0,w0,q0,[]]
case_quat_MTQ1RW_strong_prop = ["thesis_6U_quat_MTQ1RW_strongprop",           est_sat_w_all_mtq1rw,     main_sat_mtq1rw,    1,  w0,      q0,h0[0],      cov0_estimate_all_mtq_1rw,      True,  plan_mtq_goals, prop_schedule,      4500,    orb_file,all_int_cov_mtq1rw,3*(mtm_bias_on+gyro_bias_on+sun_bias_on),3*(prop_torq_on+dipole_on),3*mtq_bias_on,1e-3,0.0,"",0.0,1.0,w0,q0,h0[0]]

# tests = tests[6:]
tests = [case_quat_MTQ_strong_prop_LQR,case_quat_MTQ_strong_prop,case_quat_MTQ1RW_strong_prop,case_quat_RWMTQ_strong_prop,case_quat_RWMTQ_strong_prop_LQR]
# tests = tests[1:]
for j in tests:
    run_sim_wrapper(*j)
