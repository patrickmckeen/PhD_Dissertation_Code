...
if __name__ == '__main__':
    ...
    planner_goals = Goals({0.2:GovernorMode.NO_CONTROL,0.22+10*sec2cent:GovernorMode.BDOT_WITH_EKF,0.22+200*sec2cent:GovernorMode.PLAN_AND_TRACK_MPC}, {0.22:(PointingGoalVectorMode.NADIR,np.zeros(3)),0.22+1500*sec2cent:(PointingGoalVectorMode.ZENITH,np.zeros(3)),0.22+3000*sec2cent:(PointingGoalVectorMode.NADIR,np.zeros(3))}, {0.2:unitvecs[0]})
    ...
    plannertest = ["planner", bc_est,bc_real,1,0.01*bc_w0,bc_q0,bc_cov0_estimate,True,planner_goals,1800,"../estimation/myorb",""]
    ...
    mpc_dt = 1
    mpc_ang_limit = 3
    mpc_angwt_low = 1e3#1e4#1e4#1e4
    mpc_angwt_high = 1e10#1e12#1e5#1e12
    mpc_avwt = 1e2#1e3#1e8 #1e2
    mpc_avangwt = 1e1#1e6#1e2
    mpc_extrawt = 0
    mpc_uwt_from_plan = 1e-4#0#1e-4#1e-6
    mpc_uwt_from_prev = 0
    mpc_lqrwt_mult = 1e4#0.0#1.0
    mpc_extra_tests = 0
    mpc_tol = 1e-8
    mpc_Nplot = 0
    ...
    for j in tests:
        try:
            ...
            planner_settings = PlannerSettings(est_sat,tvlqr_len = 250,tvlqr_overlap = 100,dt_tp = 1,precalculation_time = 10,traj_overlap = 200,debug_plot_on = False,bdot_on = 1,
            ...
            planner_settings.default_traj_length = 700
            ...
            planner_settings.u_weight = 10.0#1e-1#1e-1 #0*0.0001#0.0#0.0000001
            ...
            planner_settings.angle_weight2 = 10.0#0.1#100.0
            planner_settings.angvel_weight2 = 0.1#1e-2#1# 1e-1
            planner_settings.u_weight2 = 0.001
            ...
            planner_settings.angvel_weight_tvlqr = 1.0#0.1#0.1#0.01#0#0.001#0.01#0.01#0*0.01#1#$5#0.01#0.001#0.5#0.1#0#1#0*0.000001#1e-1#0*0.0000001#0.01#0.00001
            planner_settings.u_weight_tvlqr = 0.01 #1e-2#0.001#0.00001#0.001#0.00000001#0.3#5#0.1#0.000000001#0.0000000001
            ...
            planner_settings.angvel_weight_N_tvlqr = 10.0#0.1#$1.0*10#0.1*10#0.1#0.1#0#0.01#1#1#0*0.1#0*0.01#1#5#0.01#0.001#0.5#0.1#0#1#0*0.000001#1e-1#0*0.0000001#0*0.00001
            ...
            planner_settings.useRawControlCost = False
            ...
            adcsys.planner.setVerbosity(False)
