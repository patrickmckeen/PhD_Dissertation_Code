...
def run_sim(orb,state0,real_sat,adcsys,j2000_0 = 0.22,tf=60*60*3,dt = 1,alt_title = None,rand = False):
    ...
    while t<tf:
        ...
        prev_control = control.copy()
        # breakpoint()
        control = adcsys.ADCS_update(j2000_0+(t-t0)*sec2cent,sens,gps_sens)
        ...
        try:
            print('plan av',goal_state.state[0:3]*180.0/math.pi,norm(goal_state.state[0:3]*180.0/math.pi))
        except:
            pass
        ...
        print("rot ax ang to Bbody", (180.0/np.pi)*np.arccos(np.abs(np.dot(Bbody,normalize(quat_mult(quat_inv(goal_state.state[3:7]),state[3:7])[1:])))/norm(Bbody)))
    ...
    plot_the_thing(plannedav_ECI,title = "Planned AV",xlabel='Time (s)',norm = True,ylabel = 'AV (deg/s)',xdata = np.array(t_hist),save =True,plot_now = False, save_name = folder_name+"/"+base_title+"/plan/plan_av_plot")
    ...
    plot_the_thing((plan_state_hist[:,0:3]-state_hist[:,0:3])*180.0/math.pi,title = "AV Error From Plan",xlabel='Time (s)',ylabel = 'AV Error (deg/s)', norm=True,xdata = np.array(t_hist),save =True,plot_now = False, save_name = folder_name+"/"+base_title+"/plan_v_actual/averr_from_plan_plot")
...
if __name__ == '__main__':
    ...
    bc_real = create_GPS_BC_sat_plus_1magic(real=True,rand=False,include_mtqbias = False, include_mtq_noise = False,care_about_eclipse = False,include_magicbias = False,include_magic_noise = False,use_dipole = False,use_drag = False)
    bc_est = create_GPS_BC_sat_plus_1magic( real = False, rand=False,include_mtqbias = False, include_mtq_noise = False,estimate_mtq_bias = False,include_magicbias= False, include_magic_noise=False,estimate_magic_bias = False,care_about_eclipse = False,use_dipole = False,use_drag = False)
    bc_cov0_estimate = block_diag(np.eye(3)*avcov,np.eye(3)*angcov,np.eye(3)*(1e-7*1)**2.0,np.eye(3)*(0.2*math.pi/180)**2.0,np.eye(3)*(3e-3)**2.0)
    ...
    planner_goals = Goals({0.2:GovernorMode.NO_CONTROL,0.22+10*sec2cent:GovernorMode.BDOT_WITH_EKF,0.22+50*sec2cent:GovernorMode.PLAN_AND_TRACK_MPC}, {0.22:(PointingGoalVectorMode.NADIR,np.zeros(3)),0.22+1500*sec2cent:(PointingGoalVectorMode.ZENITH,np.zeros(3)),0.22+3000*sec2cent:(PointingGoalVectorMode.NADIR,np.zeros(3))}, {0.2:unitvecs[0]})
    ...
    plannertest = ["planner", bc_est,bc_real,1,0*bc_w0,bc_q0,bc_cov0_estimate,True,planner_goals,1800,"../estimation/myorb",""]
    ...
    mpc_avwt = 1e1
    mpc_extrawt = 0
    mpc_uwt_from_plan = 0#1e-6
    mpc_uwt_from_prev = 0
    mpc_lqrwt_mult = 0
    mpc_extra_tests = 0
    mpc_tol = 1e-10
    mpc_Nplot = 0
    mpc_gain_info = [mpc_dt,mpc_ang_limit,mpc_angwt_low,mpc_angwt_high,mpc_avwt,mpc_extrawt,mpc_uwt_from_plan,mpc_uwt_from_prev,mpc_lqrwt_mult,mpc_extra_tests,mpc_tol,mpc_Nplot]#[1,10,100,1e6,1,0,1e-6,0]
    ...
    for j in tests:
        try:
            ...
            planner_settings.angle_weight = 10.0#0.1#200.0
            planner_settings.angvel_weight = 10.0#0*10.0#0.1#0.01#0*10.0#.01#01#0.000001
            ...
            planner_settings.ang_av_weight = 0.0#0*10.0#0*100.0
            planner_settings.angle_weight_N = 100.0#2000.0
            planner_settings.angvel_weight_N = 10.0#0*10.0#0.1#0*10.0
            planner_settings.av_with_mag_weight_N = 0.0
            planner_settings.ang_av_weight_N = 0.0#0*100.0

            planner_settings.angle_weight2 = 100.0#0.1#100.0
            planner_settings.angvel_weight2 = 0.1#1e-2#1# 1e-1
            planner_settings.u_weight2 = 0.1
            ...
            planner_settings.angle_weight_N2 = 100.0#1000.0
            planner_settings.angvel_weight_N2 = 0.1#1.0#0*1.0#0.0
            planner_settings.av_with_mag_weight_N2 = 0.0
            planner_settings.ang_av_weight_N2 = 0.0#1.0#0*0.2

            planner_settings.angle_weight_tvlqr = 50.0#30#0.1#10.0
            planner_settings.angvel_weight_tvlqr = 0.1#0.1#0.01#0#0.001#0.01#0.01#0*0.01#1#$5#0.01#0.001#0.5#0.1#0#1#0*0.000001#1e-1#0*0.0000001#0.01#0.00001
            planner_settings.u_weight_tvlqr = 0.05 #1e-2#0.001#0.00001#0.001#0.00000001#0.3#5#0.1#0.000000001#0.0000000001
            planner_settings.u_with_mag_weight_tvlqr = 0.0
            planner_settings.av_with_mag_weight_tvlqr = 0.0#0.0#1.0#5.0#0#0.005#0.01
            planner_settings.ang_av_weight_tvlqr = 0.02#0*2#0.05#0.01#0.5#0.5#0#10#0.01#0.1#0*0.1#0*10#1000#-0.1#0.0001#0.01
            planner_settings.angle_weight_N_tvlqr = 500.0#1*500#1#500#200#30#20.0
            planner_settings.angvel_weight_N_tvlqr = 1.0#$1.0*10#0.1*10#0.1#0.1#0#0.01#1#1#0*0.1#0*0.01#1#5#0.01#0.001#0.5#0.1#0#1#0*0.000001#1e-1#0*0.0000001#0*0.00001
            planner_settings.av_with_mag_weight_N_tvlqr = 0.0#1.0
            planner_settings.ang_av_weight_N_tvlqr = 0.2#0.1#0*2#5.0#0#0.05*100#0*0.01#0.01#0.5#0.5#0#10#0.01#0.1#0.01#0.0
            ...
            planner_settings.control_limit_scale = 0.7
