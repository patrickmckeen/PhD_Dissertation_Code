...
#
# suns = [SunSensorPair(unitvecs[j],noise_sun,sun_eff,has_bias = False,bias = real_sun_bias0[j],use_noise = True,bias_std_rate = sun_bsr) for j in range(3)]
# suns2 = [SunSensor(-unitvecs[j],noise_sun,sun_eff,has_bias = False,bias = real_sun_bias0[j+2],use_noise = True,bias_std_rate = sun_bsr) for j in range(3)]
...
# suns_est = [SunSensorPair(unitvecs[j],noise_sun,sun_eff,has_bias = False,bias = 0,use_noise = False,bias_std_rate = sun_bsr,estimate_bias = False) for j in range(3)]
# suns2_est = [SunSensor(-unitvecs[j],noise_sun,sun_eff,has_bias = False,bias = 0,use_noise = False,bias_std_rate = sun_bsr,estimate_bias = False) for j in range(3)]
...
estimate[3] = 1
cov_estimate = block_diag(np.eye(3)*(1.0*math.pi/180.0)*2.0,np.eye(4)*3,np.eye(3)*0.2**2.0,np.eye(3)*1e-8,np.eye(3)*0.2*0.2,np.eye(3)*0.1*0.1)
...
estimate = np.zeros(7+3+3)
estimate[3:7] = 0.5
estimate[0:3] = 0.00001
estimate[7:10] = 0.0001
estimate[10:13] = 0.0001
cov_estimate = block_diag(np.eye(3)*(0.5*math.pi/180.0)**2.0,np.eye(3)*3,(0.5*math.pi/180)**2.0*np.eye(3),(0.3*0.05)**2.0*np.eye(3))
int_cov =  block_diag(np.eye(3)*(0.001*math.pi/180.0)**2.0,0.001*np.eye(3),np.eye(3)*gyro_bsr**2.0,np.eye(3)*sun_bsr**2.0)
#
# estimate = np.zeros(7+3+3)
# estimate[3:7] = 0.5
# estimate[0:3] = 0.0001
# estimate[7:10] = 0.0001
# estimate[10:13] = 0.0001
# cov_estimate = block_diag(np.eye(3)*(1.0*math.pi/180.0)**2.0,np.eye(3),100*np.eye(3)*mtq_bias0_std**2.0,100*np.eye(3)*gyro_bias0_std**2.0)
# int_cov =  block_diag(np.eye(3)*(0.0001*math.pi/180.0)**2.0,np.eye(3)*0.0001,2*np.eye(3)*mtq_bsr**2.0,np.eye(3)*gyro_bsr**2.0)
