...
class MTQ_W_RW_PD_MINE(ControlMode):
    ...
    def find_actuation(self, state, os, osp1, goal_state, prev_goal,next_goal, sens,planner_params,is_fake):
        ...
        return self.general_ctrl_from_torq(state,w_err,os,vecs,self.params.RW_bias_h,np.ones(self.rw_N)*self.params.c_gain,base_torq,np.ones(self.mtq_N)*1e3,np.ones(self.magic_N)*1e-8,np.ones(self.rw_N)*1e5,wdesdot = np.zeros(3))
