...
class WisniewskiSliding(ControlMode):
    def __init__(self,gain_info,sat,maintain_RW = True,include_disturbances=False,quatset=False,quatset_type = "B",calc_av_from_quat = False,include_rotational_motion = True):
        if not quatset:
            ModeName = GovernorMode.WISNIEWSKI_SLIDING
        else:
            if quatset_type == "B":
                ...
                ModeName = GovernorMode.WISNIEWSKI_SLIDING_QUATSET_BS
            else:
                raise ValueError("incorrect quatset class of WisniewskiSliding")
    ...
    def scalc(self,qerr,werr,state,save = False,do_print=False):
        ...
        s1 = werr@self.sat.J@self.params.Lambda_All
        s2 = qerr[1:]@self.params.Lambda_q@self.params.Lambda_All
        s3 = np.nan*np.zeros(3)
        if do_print:
            print('s ',s)
            print('s1 ',s1)
            print('s2 ',s2)
    ...
    def sdotcalc_noact(self,qerr,werr,state,vecs,save = False,do_print=False):
        ...
        sdot = 0.5*(qerr[0]*werr + np.cross(qerr[1:],werr))@self.params.Lambda_q@self.params.Lambda_All
        if do_print:
            tt=  qerr[1:]@self.params.Lambda_q@self.params.Lambda_All
            ttd = 0.5*( qerr[0]*werr + np.cross(qerr[1:],werr))@self.params.Lambda_q@self.params.Lambda_All
    ...
    def baseline_actuation(self,state,qerr,werr,vecs,other_err=None,dt = 1,save = False):
        ...
        base_torq -= s@self.params.Lambda_s#@self.params.Lambda_All
        # udes = base_torq
    ...
    def find_actuation(self, state, os, osp1, goal_state, prev_goal,next_goal, sens,planner_params,is_fake):
        ...
        if self.modename is GovernorMode.WISNIEWSKI_SLIDING:
            ...
            elif self.modename is GovernorMode.WISNIEWSKI_SLIDING_QUATSET_BS:
            ...
            #     pass
                ...
                raise ValueError("Incorrect mode for Wisniewksi")
