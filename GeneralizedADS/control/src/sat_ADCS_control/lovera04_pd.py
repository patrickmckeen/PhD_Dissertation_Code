...
class Lovera04(ControlMode):
    def __init__(self,gain_info,sat,maintain_RW = True,include_disturbances=False,quatset=False,quatset_type = "B",calc_av_from_quat = False,include_rotational_motion = False):
        if not quatset:
            ModeName = GovernorMode.LOVERA4_MAG_PD
        else:
            if quatset_type == "B":
                ModeName = GovernorMode.LOVERA4_MAG_PD_QUATSET_B
            elif quatset_type == "Ang":
                ModeName = GovernorMode.LOVERA4_MAG_PD_QUATSET_ANG
            elif quatset_type[:-1] == "Lyap":
                ModeName = GovernorMode.LOVERA4_MAG_PD_QUATSET_LYAP
            elif quatset_type[:-1] == "LyapRmat":
                ModeName = GovernorMode.LOVERA4_MAG_PD_QUATSET_LYAPR
    ...
    def baseline_actuation(self,state,qerr,werr,vecs,other_err=None):
        base_torq = -(self.params.gain_eps*self.params.gain_eps*self.params.kp_gain*qerr[1:] + self.params.kv_gain*self.params.gain_eps*werr@self.sat.J)
        # print('q',(self.params.gain_eps*self.params.gain_eps*self.params.kp_gain*qerr[1:]),norm((self.params.gain_eps*self.params.gain_eps*self.params.kp_gain*qerr[1:])))
        # print('H',(self.params.kv_gain*self.params.gain_eps*werr)@self.sat.J,norm((self.params.kv_gain*self.params.gain_eps*werr)@self.sat.J))
        ...
        # print(self.params.gain_eps)
        # print(self.params.kp_gain)
        # print(self.params.kv_gain)
        mtq_cmd = np.cross(vecs["b"],base_torq)/(norm(vecs["b"])**2.0)
        # print('inside')
        # print('b',vecs["b"])
        # print('base_torq',base_torq,(180.0/np.pi)*np.arccos(np.dot(normalize(vecs["b"]),normalize(base_torq))),0.0)
        # print('q',qerr,(180.0/np.pi)*np.arccos(np.dot(normalize(vecs["b"]),normalize(qerr[1:]))),(180.0/np.pi)*np.arccos(np.dot(normalize(base_torq),normalize(qerr[1:]))))
        # print('H',werr,(180.0/np.pi)*np.arccos(np.dot(normalize(vecs["b"]),normalize(werr@self.sat.J))),(180.0/np.pi)*np.arccos(np.dot(normalize(base_torq),normalize(werr@self.sat.J))))
        # # print(mtq_cmd)
        # print('unit b ', normalize(vecs["b"]))
        # print('unit t ',normalize(base_torq))
        # print('unit H ',normalize(werr@self.sat.J))
        # print('unit q ',normalize(qerr[1:]))
    ...
    def find_actuation(self, state, os, osp1, goal_state, prev_goal,next_goal, sens,planner_params,is_fake):
        ...
        if self.modename is GovernorMode.LOVERA4_MAG_PD:
            ...
            if self.modename is GovernorMode.LOVERA4_MAG_PD_QUATSET_B:
                w_err,q_err = self.perpB_err(state,goal_state,next_goal,(osp1.J2000-os.J2000)*cent2sec,vecs["b"])
            elif self.modename is GovernorMode.LOVERA4_MAG_PD_QUATSET_ANG:
                w_err,q_err = self.minang_err(state,goal_state,next_goal,(osp1.J2000-os.J2000)*cent2sec)
            elif self.modename is GovernorMode.LOVERA4_MAG_PD_QUATSET_LYAP:
                w_err,q_err = self.minLyap_err(state,goal_state,vecs,next_goal,(osp1.J2000-os.J2000)*cent2sec)
            elif self.modename is GovernorMode.LOVERA4_MAG_PD_QUATSET_LYAPR:
                w_err,q_err = self.minLyapR_err(state,goal_state,vecs,next_goal,(osp1.J2000-os.J2000)*cent2sec)
