...
class WisniewskiTwisting2(ControlMode):
    def __init__(self,gain_info,sat,maintain_RW = True,include_disturbances=False,quatset=False,quatset_type = "B",calc_av_from_quat = False,include_rotational_motion = True):
        if not quatset:
            ModeName = GovernorMode.WISNIEWSKI_TWISTING2
        else:
            if quatset_type == "B":
                ModeName = GovernorMode.WISNIEWSKI_TWISTING2_QUATSET_B
            elif quatset_type == "Ang":
                ModeName = GovernorMode.WISNIEWSKI_TWISTING2_QUATSET_ANG
            elif quatset_type[:-1] == "Lyap":
                ModeName = GovernorMode.WISNIEWSKI_TWISTING2_QUATSET_LYAP
            elif quatset_type == "S":
                ModeName = GovernorMode.WISNIEWSKI_TWISTING2_QUATSET_MINS
            elif quatset_type[:-1] == "LyapRmat":
                ModeName = GovernorMode.WISNIEWSKI_TWISTING2_QUATSET_LYAPR
            else:
                raise ValueError("incorrect quatset class of WisniewskiTwisting2")
    ...
    def baseline_actuation(self,state,qerr,werr,vecs,other_err=None,dt = 1,save = False):
        ...
        if self.sat.number_RW > 0:
            ang_mom += state[7:self.sat.state_len]@self.RWaxes
        # s1d = (-np.cross(state[0:3],ang_mom) - np.cross(vecs["b"],u_noB) + self.saved_dist)@self.params.Lambda_All
        ...
        #
        # s1dd = (-np.cross(s1d@self.params.inv_Lambda_All@self.sat.invJ,ang_mom) - np.cross(state[0:3],s1d@self.params.inv_Lambda_All) - np.cross(np.cross(-state[0:3],vecs["b"]),u_noB) - np.cross(state[0:3],self.saved_dist))@self.params.Lambda_All
        ...
        #     self.torq = -np.cross(vecs["b"],u_noB) + self.saved_dist
    ...
    def find_actuation(self, state, os, osp1, goal_state, prev_goal,next_goal, sens,planner_params,is_fake):
        ...
        if self.modename is GovernorMode.WISNIEWSKI_TWISTING2:
            ...
            if self.modename is GovernorMode.wisniewski_twisting2_goals_QUATSET_B:
                w_err,q_err = self.perpB_err(state,goal_state,next_goal,(osp1.J2000-os.J2000)*cent2sec,vecs["b"])
            elif self.modename is GovernorMode.WISNIEWSKI_TWISTING2_QUATSET_ANG:
                w_err,q_err = self.minang_err(state,goal_state,next_goal,(osp1.J2000-os.J2000)*cent2sec)
            elif self.modename is GovernorMode.WISNIEWSKI_TWISTING2_QUATSET_LYAP:
            ...
            elif self.modename is GovernorMode.WISNIEWSKI_TWISTING2_QUATSET_LYAPR:
            ...
            #     pass
                ...
                raise ValueError("Incorrect mode for Wisniewksi twisting2")
