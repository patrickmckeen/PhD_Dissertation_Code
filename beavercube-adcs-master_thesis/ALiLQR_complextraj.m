...
swpoint = 0.01*(rad2deg(1))^2;%100/max(abs(wdes)+0.01,[],'all')^2;%importance of angular velocity during regular times
sv1 = 1e1;%1e8; %importance of aoreitnation or angular difference
sv2 = 1e0;
su = 2e3;%1/max(abs(mdes)+1e-6,[],'all'); %importance of u
sratioslew = 0.005;%0.0001;%ratio of importance of params (except ang vel) during slew or off-freq
...
if reInitialize
    ...
    load('matfiles/r_ECI.mat');
    load('matfiles/v_ECI.mat');
    load('matfiles/B_ECI.mat');
    ...
    satAlignVector1 = @(k) unitf([0 0 -1].'*(double(k<(passtime+passdur/2)) + (k>=(passtime+passdur/2)).*(k<proptime).*(1-((k-(passtime+passdur/2))/(proptime-(passtime+passdur/2))))) + [1 0 0].'*(double(k>=proptime) + (k>=(passtime+passdur/2)).*(k<proptime).*(((k-(passtime+passdur/2))/(proptime-(passtime+passdur/2))))));
    satAlignVector2 = [0 1 0].';
    ECIAlignVector1 = @(k,rk,vk) unitf((max(k<(passtime+passdur/2),(k>=(passtime+passdur/2))*(1-((k-(passtime+passdur/2))/(proptime-(passtime+passdur/2))))))*(k<proptime)*(6370*Rset(:,passtime)/norm(Rset(:,passtime)) - rk)/norm((6370*Rset(:,passtime)/norm(Rset(:,passtime)) - rk)) + (k>=(passtime+passdur/2))*(max(k>=proptime,(k<proptime)*(k-(passtime+passdur/2))/(proptime-(passtime+passdur/2))))*-vk./vecnorm(vk));
    ECIAlignVector2 = @(k,rk,vk) -rk./vecnorm(rk);



    %DEFINE COMMAND MODULE CONSTANT PARAMpasETERS%
    qf_desired = zeros(sim_length,4);
    qf_guess = zeros(sim_length,4);
    rotlim = 0.5;
    for i=1:sim_length
        q_command = qcommand_PM(satAlignVector1(i), satAlignVector2, ECIAlignVector1(i,r_ECI(i, :).',v_ECI(i, :).'), ECIAlignVector2(i,r_ECI(i, :).',v_ECI(i, :).'));
        q_gc = qcommand_PM(satAlignVector1(i), (R_z(360*i*0/300)*satAlignVector2),ECIAlignVector1(i,r_ECI(i, :).',v_ECI(i, :).'), ECIAlignVector2(i,r_ECI(i, :).',v_ECI(i, :).'));
        if i > 1
...
function Qk = Q(k,rk,vk)
    ...
    global Nslew sv1 swpoint sv2 rNslew vNslew swslew sratioslew dt freq passtime proptime passdur decaytime
    
    if (abs(k-passtime) <= passdur/2) || k>=proptime
%     if k >= Nslew
%         a = [0 0 0].';
%         b = rk;
%         vk
...
%         Qkqq = sv1*(e2*e2.') + sv2*(e3*e3.');
        ...
        timeoff = min(max(0,min([abs(k-passtime) - passdur/2, proptime - k])),decaytime);
        Qk = Qk*(sratioslew*(timeoff/decaytime) + 1*(1 - timeoff/decaytime));
...
%     Urand = umax.*((1-2*rand(m,1))/10).*sin((0:N-1).*((0.5*rand(m,1)).^(2))/(N/10) + rand(m,1)*2*pi);
    ...
    if useGuess
        ...
        if useGraph
            ...
            figure(figlabel);
            subplot(3,1,1)
            plot(quatrotate(quatconj(Xset(4:7,:).'),satAlignVector1([1:N]).'));
            if useGuess
                subplot(3,1,2)
