...
useGuess = false; %can give trajGuess--otherwise starts with a random trajectory
...
useGuessOmega = false; %set an omega for the desired trajectory. If true, it generates a suggested velocity profile based on its desired orienatation. If false, the desired angular velocity is assumed to be zeros, so the cost function weights against higher velocity.
...
swslew = 0.000001;%0.0001;%10/max(abs(wdes)+0.01,[],'all')^2; %importance of angular velocity during slew or off-freq
swpoint = 0.0001*(rad2deg(1))^2;%100/max(abs(wdes)+0.01,[],'all')^2;%importance of angular velocity during regular times
sv1 = 500;%1e8; %importance of aoreitnation or angular difference
sv2 = 1e0;
su = 500;%1/max(abs(mdes)+1e-6,[],'all'); %importance of u
sratioslew = 0.0001;%0.0001;%ratio of importance of params (except ang vel) during slew or off-freq
sslack = 1e8;%max([swslew swpoint sv1 sv2 su sratioslew]); %importance of slacks in cost %don't actualyl want slacks to be too important to cost--they will be removed by constraining them to 0, and by allowing their cost to be comparatively small, a more ideal trajectory is found initially.
...
if reInitialize
    ...
    load('C:\Users\PMcKeen\Documents\Research\Beavercube\beavercube-adcs-master\matfiles\r_ECI.mat');
    load('C:\Users\PMcKeen\Documents\Research\Beavercube\beavercube-adcs-master\matfiles\v_ECI.mat');
    load('C:\Users\PMcKeen\Documents\Research\Beavercube\beavercube-adcs-master\matfiles\B_ECI.mat');
    load('C:\Users\PMcKeen\Documents\Research\Beavercube\beavercube-adcs-master\matfiles\sun_ECI.mat');
    ...
    w_mag_mean = deg2rad(0.1);
    w_mag_var = deg2rad(0.01);
    w_bias_mean = 0;
    ...
    x0(1:3) = max(min(x0(1:3),0.9*wmax),-0.9*wmax);
    x0(1:3) = x0(1:3)*min(1,0.9*wmax/norm(x0(1:3)));
%     x0 = [0.00067 0.00067 0.00046 -0.1096 -0.5967 0.5336 0.5893].';
...
load('C:\Users\PMcKeen\Documents\Research\Beavercube\beavercube-adcs-master\matfiles\g_coefs.mat');
load('C:\Users\PMcKeen\Documents\Research\Beavercube\beavercube-adcs-master\matfiles\h_coefs.mat');
global g
global h
...
%% run the code
tic
[Xset,Uset,lambdaSet,Kset,Pset,slackX,slackU] = trajOpt(x0,Rset,Vset,Xdesset,trajGuess,Bset);
timetook = toc
...
function ci = ineqConstraints(k,x,u)  %ineq before eq
    ...
    if k == N
        ci = zeros(13,1);
    elseif k<N && k >= 0
        ci = [u-umax;-u-umax;x(1:3)-wmax;-x(1:3)-wmax;sum(x(1:3).^2)-wmax^2];
    else
...
function [icku,ickx] = ineqConstraintsJacobians(k,x,u)  %ineq before eq
    ...
    if k == N
        icku = zeros(numc,m);
        ickx = [zeros(6,n);eye(3,3) zeros(3,4);-eye(3,3) zeros(3,4); x(1:3).' zeros(1,4)];
    elseif k<N && k>=0
        icku = [eye(3,3);-eye(3,3);zeros(7,3)];
        ickx = [zeros(6,n);eye(3,3) zeros(3,4);-eye(3,3) zeros(3,4); x(1:3).' zeros(1,4)];
    else
        error('invalid time index')
    end
...
%                 dang = (-2*(la*a.' + lb*b.'))*Wmat(qk);
                dang = (-2*(la*a.' + lb*b.'))*Wmat(qk)/mag/(sqrt(1-mag^2)+eps);
...
function xkp1 = rk4(xk,uk,rk,tk,dtl)
    ...
    k1(end-3:end) = k1(end-3:end)/max(1,norm(k1(end-3:end)));
    k2 = dynamics(xk+0.5*dtl*k1,uk,rk,tk);
    k2(end-3:end) = k2(end-3:end)/max(1,norm(k2(end-3:end)));
    k3 = dynamics(xk+0.5*dtl*k2,uk,rk,tk);
    k3(end-3:end) = k3(end-3:end)/max(1,norm(k3(end-3:end)));
    k4 = dynamics(xk+dtl*k3,uk,rk,tk);
    k4(end-3:end) = k4(end-3:end)/max(1,norm(k4(end-3:end)));
    xkp1 = xk + (dtl/6)*(k1+2*k2+2*k3+k4);
    xkp1(end-3:end) = xkp1(end-3:end)/max(1,norm(xkp1(end-3:end)));
end
...
function [Pset,Kset,dset,delV,rho,drho] = backwardpass(Xset,Uset,Rset,Vset,Xdesset,lambdaSet,rho,drho,mu,slackset,useSlacks)
    global N m n QN regScale regMin dt numDOF numc
    if useSlacks
...
function [newXset,newUset,newLA,rho,drho,newslackset] = forwardpass(Xset,Uset,Kset,Rset,Vset,dset,delV,LA,lambdaSet,rho,drho,mu,Xdesset,slackset,useSlacks)
    ...
    while ((z<=beta1 || z>beta2) && (newLA>=LA))
        if iter > maxLsIter
            ...
            newLA = cost(newXset,newUset,Rset,Vset,Xdesset,lambdaSet,mu,slackset,useSlacks);
            z = 0;
            alph = 0;
            exp = 0;
            rho = rho + regBump
...
%             nnz(isnan(newUset))
        ...
        newLA = cost(newXset,newUset,Rset,Vset,Xdesset,lambdaSet,mu,newslackset,useSlacks);
        if newLA <0
...
function [Uset] = bdot(Bset,Rset,x0)
    global n useGuess N dt J umax wmax bdotgain
    newX = zeros(n,N);
    newX(:,1) = x0;
    uk = -bdotgain*(-cross(newX(1:3,1),rotT(newX(end-3:end,1))*Bset(:,1)) + rotT(newX(end-3:end,1))*(Bset(:,2)-Bset(:,1))/dt);
%     uk = uk*norm(umax)/norm(uk);
%     uk = uk
%     uk = uk*norm(newX(1:3,1))/norm(wmax);
    uk = max(min(uk,umax),-umax);
    Uset(:,1) = uk;
    for k = 2:N-1
        %newX(:,k) = rk4(newX(:,k-1),Uset(:,k-1),Rset(:,k-1),k*dt); 
        xk = rk4(newX(:,k-1),Uset(:,k-1),Rset(:,dt*(k-2)+1),dt*(k-2)+1,dt); 
        xk(end-3:end) = xk(end-3:end)/norm(xk(end-3:end));
        newX(:,k) = xk;
        uk = -bdotgain*(-cross(newX(1:3,k),rotT(newX(end-3:end,k))*Bset(:,k)) + rotT(newX(end-3:end,k))*(Bset(:,k+1)-Bset(:,k))/dt);
%         uk = uk*norm(umax)/norm(uk);
%         uk = uk*norm(xk(1:3))/norm(wmax);
        uk = max(min(uk,umax),-umax);
        Uset(:,k) = uk;
%         if any(isnan(newX(:,k)))
%             rk4(newX(:,k-1),Uset(:,k-1),Rset(:,dt*(k-1)),(k-1)*dt)
%         end
        ...
        %newX(:,k) = rk4(newX(:,k-1),Uset(:,k-1),Rset(:,k-1),k*dt); 
%     xk = rk4(newX(:,N-1),Uset(:,N-1),Rset(:,dt*(N-2)+1),dt*(N-2)+1,dt); 
%     xk(end-3:end) = xk(end-3:end)/norm(xk(end-3:end));
%     newX(:,N) = xk;
%     figure;
%     subplot(2,1,1)
...
%     plot(newX(1:3,:).')
%     hold on
%     plot(vecnorm(newX(1:3,:)))
end

function [Xset,Uset,lambdaSet,Kset,Pset,slackX,slackU] = trajOpt(x0,Rset,Vset,Xdesset,trajGuess,Bset)
...
%     Urand = umax.*(1-2*rand(m,N-1))/10;
%     Urand = (umax/100).*sin((ones(3,1)*(0:N-1)).*2*pi./(N./[2 2 2].'));
    Urand = bdot(Bset,Rset,x0);
    Urand0 = Urand;
%     Urand = umax.*((1-2*rand(m,1))/10).*sin((0:N-1).*((0.5*rand(m,1)).^(2))/(N/10) + rand(m,1)*2*pi);
    if ~usePresetU
        Uset = Urand;
        U0 = Uset;
    else
...
%             drho
            ...
            if useExpAngle
                approxAng = rad2deg(log(sqrt(LAnc/N/sv1)))
            elseif useAngSquared
                approxAng = rad2deg(sqrt(sqrt(LAnc/N/sv1)))
            else
                approxAng = rad2deg(sqrt(LAnc/N/sv1))
            end
            if useGraph
                figure(figlabel);
                subplot(3,1,1)
